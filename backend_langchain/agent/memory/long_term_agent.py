...
写路径图见 `agent.graphs.agents.memory`（trigger → extract → apply）。
"""
...
from agent.memory.long_term_trigger import memory_agent_enabled
...
def _get_graph():
    global _graph
    if _graph is None:
        from agent.graphs.agents.memory import build_memory_agent_graph
...
def format_retrieved_memories(user_id: int, query: str) -> str:
    ...
    if not q or user_id <= 0:
        return ""
    from agent.memory.long_term_trigger import should_retrieve_memory
