...
3. 若以上均未设置：自动读取 backend_langchain/agent/mcp/mcp_servers.json（存在则加载）。

请将 agent/mcp/mcp_servers.example.json 复制为 agent/mcp/mcp_servers.json，写入真实 Token（该文件已 .gitignore）。
联网搜索由 agent/rag/web_search.forced_web_search 直连百炼 WebSearch MCP（DASHSCOPE_API_KEY），
...
_DEFAULT_MCP_CONFIG = "agent/mcp/mcp_servers.json"
